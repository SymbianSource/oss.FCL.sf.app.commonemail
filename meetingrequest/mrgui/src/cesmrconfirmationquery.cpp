...
TBool CESMRConfirmationQuery::ExecuteLD()
    ...
    switch ( iType )
        ...
        case EESMRAttachments:
            ...
            break;
        case EESMRCancelDownloadingAttachment:
            {
            noteText =
                iEnv->AllocReadResourceAsDes16LC(
                        R_QTN_MEET_REQ_VIEWER_CANCEL_DOWNLOAD_QUERY );
            }
            break;
        case EESMRCancelDownloadingAllAttachments:
            {
            noteText =
                iEnv->AllocReadResourceAsDes16LC(
                        R_QTN_MEET_REQ_VIEWER_CANCEL_DOWNLOADS_QUERY );
            }
            break;
...
// -----------------------------------------------------------------------------
//
TBool CESMRConfirmationQuery::ExecuteL( const TDesC& aLocation )
    {
    FUNC_LOG;
