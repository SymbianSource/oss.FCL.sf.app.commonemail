...
enum TESMRSendMRFSMailTaskPanic
    {
    EESMRSendMRFSMailInvalidMethod,
    EESMRSendMRFSAttendeesMissing,
    EESMRSendMRNoEntriesToSend
    };
...
void CESMRSendMRFSMailTask::ConstructMailL()
    ...
    if ( CCalEntry::EMethodCancel == entryMethod )
        ...
        SaveICalToFileL( *calendarPart, KCancelFile );
        iEmailMgr->CreateTextCalendarPartL(method, KCancelFile);
        //</cmail>
        }
    else
        {
...
void CESMRSendMRFSMailTask::GetEntriesToBeSentL()
	...
	if ( EESMRCmdSendMRUpdate == iCommand )
		...
		if ( MESMRCalEntry::EESMRAllInSeries == ESMREntry().RecurrenceModRule() )
			...
			// attendees calendar.			
			CalDbMgr().NormalDbEntryView()->FetchL(
					mainEntry->UidL(),
					iEntriesToSend );
